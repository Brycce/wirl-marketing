  ...
  return (
    <svg viewBox="0 0 560 650" className="tb-art" role="img" aria-label="Three steps: a terminal installs Wirl in Claude Code; a browser at app.wirl.dev asks to connect Claude Code to harbor.co with a code, and a hand presses Approve; the agent is asked to deploy and answers with the harbor--supplier-payments.wirl.run link." strokeLinecap="round" strokeLinejoin="round">
      ...
      <Term x={56} y={24} w={488} h={124} barH={34}>
        <text x={112} y={46} fontSize={14} fill="#B9C6BD">Terminal</text>
      ...
      <Num x={50} y={30} n={1} tilt={-8} />

      {/* 2. Approve, once. The same code shows in the terminal tucked behind. */}
      <g>
        <Term x={398} y={196} w={146} h={122} barH={28}>
          <text className="mono" x={458} y={250} fontSize={13} fill="#B9C6BD">code</text>
          <text className="mono" x={458} y={276} fontSize={14} fill={K.sun} fontWeight={700}>KTQM-</text>
          <text className="mono" x={458} y={296} fontSize={14} fill={K.sun} fontWeight={700}>4821</text>
        </Term>
        <Win x={20} y={184} w={430} h={226} bar={K.sun} barH={40}>
          <LinkBar x={78} y={189} w={200} h={30} text="app.wirl.dev" size={15} />
          <text x={44} y={266} fontSize={19} fontWeight={800} fill={K.ink}>Connect Claude Code to harbor.co?</text>
          <rect x={44} y={292} width={196} height={50} rx={12} fill={K.wash} stroke={K.ink} strokeWidth={2} strokeDasharray="6 5" />
          <text className="mono" x={142} y={326} fontSize={24} fontWeight={700} fill={K.ink} textAnchor="middle" letterSpacing="0.04em">KTQM-4821</text>
          <text x={44} y={378} fontSize={15} fontWeight={600} fill="#5E5A4C">Same code as your terminal.</text>
          <Btn x={272} y={298} w={150} h={40} faceClass="tb-s-approve" label="Approve" size={17} />
          <CheckBadge x={286} y={292} r={12} className="tb-s-check" />
        </Win>
        <Hand x={380} y={322} s={1} className="tb-s-hand" />
        <Num x={24} y={192} n={2} tilt={6} />
      </g>

      {/* 3. Deploy */}
      <g>
        <Term x={56} y={454} w={488} h={170} barH={36}>
          <text x={126} y={477.5} fontSize={15} fill={K.termText}>Claude Code</text>
        ...
        <LogoSticker agent="claude" x={104} y={472} size={24} uid="steps-chat" />
